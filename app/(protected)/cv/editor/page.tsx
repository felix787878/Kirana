...
export default function CvPercobaanEditorPage() {
  ...
  const accentKey = searchParams.get("a") ?? searchParams.get("accent") ?? "teal";
  ...
  function startPhotoDrag(clientX: number, clientY: number) {
  ...
  useEffect(() => {
    function onMouseMove(event: MouseEvent) {
      ...
      setPhotoOffsetX(clampPhotoOffset(drag.originX + deltaX / 3));
      setPhotoOffsetY(clampPhotoOffset(drag.originY + deltaY / 3));
    }
  ...
  function buildHistoryEntry(updatedAtMs: number): CvHistoryEntry {
    ...
    return {
      id: entryId || `k${updatedAtMs.toString(36)}`,
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
        <aside className="order-1 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm xl:order-2 xl:sticky xl:top-20 xl:h-fit">
          <div className="grid min-h-[430px] grid-cols-[108px_minmax(0,1fr)]">
            <div className="px-2 py-3 text-white" style={{ backgroundColor: accent }}>
              <div
                ...
                onMouseDown={(e) => startPhotoDrag(e.clientX, e.clientY)}
                onTouchStart={(e) => {
                  if (e.touches.length > 0) startPhotoDrag(e.touches[0].clientX, e.touches[0].clientY);
                }}
              >
              ...
              <div className="mt-2 border-t border-white/80 pt-2.5">
                <p className="text-[9px] font-bold uppercase leading-tight tracking-normal text-white/95">
                  Keterampilan
                </p>
              ...
              {extraSections.includes("bahasa") ? (
                <div className="mt-4 border-t border-white/80 pt-2.5">
                  <p className="text-[9px] font-bold uppercase leading-tight tracking-normal text-white/95">Bahasa</p>
              ...
              {extraSections.includes("hobi") ? (
                <div className="mt-4 border-t border-white/80 pt-2.5">
                  <p className="text-[9px] font-bold uppercase leading-tight tracking-normal text-white/95">Hobi</p>
                  <ul className="mt-1.5 space-y-0.5 text-[9px] leading-snug text-white/95">
                    {(hobbyText.trim()
                      ? hobbyText.split("\n").flatMap((line) =>
                          line
                            .split(/[;,]/)
                      ...
                      .slice(0, 6)
                      .map((line, idx) => (
                        <li key={`${line}-${idx}`} className="line-clamp-2">
                          - {line}
                        </li>
                      ))}
                    {!hobbyText.trim() ? (
                      <li className="text-white/75">Tambahkan hobi di langkah Bagian lainnya.</li>
                    ) : null}
                  </ul>
                </div>
              ) : null}
